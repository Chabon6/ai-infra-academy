...
function renderTerm(id,scroll=false){
 ...
 $('#detail').innerHTML=`<div class="eyebrow">${LAYERS[t.layer][0]} / ${t.id.toUpperCase()}</div><h3 tabindex="-1" id="detail-title">${t.title}</h3><p class="english">${t.en}</p><p class="takeaway">${t.one}</p><p>${t.body}</p><div class="analogy"><b>換個方式理解</b>${t.analogy}</div><div class="roles"><div><h4>與台積電的關係</h4><p>${t.tsmc}</p></div><div><h4>與輝達的關係</h4><p>${t.nvidia}</p></div></div><div class="invest"><h4>讀投資新聞時，可先追問</h4><p>${t.invest}</p></div><div class="pitfall"><h4>容易混淆</h4><p>${t.pitfall}</p></div><div class="companies"><h4>台股代表公司（依產品領域列舉）</h4><div class="table-wrap"><table><thead><tr><th>公司／代號</th><th>市場</th><th>本頁對應角色</th></tr></thead><tbody>${companyRows(t.companies)}</tbody></table></div><p class="small">產品領域相符，不表示已供貨給特定平台。只有官方明示的合作，才在本頁解讀為合作；未列公司也不表示不在產業鏈內。</p></div><div class="source-links">${t.sources.map(sourceLink).join('')}</div><div class="related"><span>接著理解</span>${t.related.map(id=>`<button data-term="${id}" data-scroll="true">${termById[id].title} ↗</button>`).join('')}</div>`;
