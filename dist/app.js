...
function companyRows(t){return t.companies.map(id=>{const c=COMPANIES[id],role=t.companyRoles?.[id]||c[3],keys=t.companySources?.[id]||[c[4]];return `<tr><td><a target="_blank" rel="noopener noreferrer" href="${SOURCES[keys[0]][1]}">${c[0]} ↗</a><span class="code">${c[1]}</span></td><td>${c[2]}</td><td>${escapeHTML(role)}${keys.map(key=>`<a class="company-evidence" href="${SOURCES[key][1]}" target="_blank" rel="noopener noreferrer">${escapeHTML(SOURCES[key][0])} ↗</a>`).join('')}</td></tr>`}).join('')}

function renderTerm(id,scroll=false){
 const t=termById[id];if(!t)return;selectedTerm=id;selectedLayer=t.layer;renderLayers();
 $('#detail').innerHTML=`<div class="eyebrow">${LAYERS[t.layer][0]} / ${t.id.toUpperCase()}</div><h3 tabindex="-1" id="detail-title">${t.title}</h3><p class="english">${t.en}</p><p class="takeaway">${t.one}</p><p>${t.body}</p><div class="analogy"><b>換個方式理解</b>${t.analogy}</div><div class="roles"><div><h4>與台積電的關係</h4><p>${t.tsmc}</p></div><div><h4>與輝達的關係</h4><p>${t.nvidia}</p></div></div><div class="invest"><h4>讀投資新聞時，可先追問</h4><p>${t.invest}</p></div><div class="pitfall"><h4>容易混淆</h4><p>${t.pitfall}</p></div><div class="companies"><h4>台股代表公司與實際分工</h4><p class="company-note">${escapeHTML(t.companyNote||"依公開產品列舉；同一條目可能包含製造、材料、設備或系統整合等不同分工。")}</p><div class="table-wrap"><table><thead><tr><th>公司／代號</th><th>市場</th><th>在這個條目中的角色</th></tr></thead><tbody>${companyRows(t)}</tbody></table></div><p class="small">公司對照查核：2026-09-09；掛牌資料：2026-09-08。產品相符不表示已供貨給特定平台；未列公司也不表示不在產業鏈內。</p></div><div class="source-links">${t.sources.map(sourceLink).join('')}</div><div class="related"><span>接著理解</span>${t.related.map(id=>`<button data-term="${id}" data-scroll="true">${termById[id].title} ↗</button>`).join('')}</div>`;
...
document.addEventListener('click',e=>{
 const term=e.target.closest('[data-term]');if(term)renderTerm(term.dataset.term,term.dataset.scroll==='true'||window.mobileExperience);
...
const stackItems=[['gpu','運算裸晶 ＋ HBM','最上方：運算晶片和記憶體靠近配置。HBM 是堆疊式 DRAM 記憶體，不是下方的載板。'],['interposer','中介層 / Interposer','中介層：連接裸晶與 HBM 的高密度結構；CoWoS-S、R、L 的做法不同。'],['substrate','IC 載板 / Package Substrate','IC 載板：承接封裝，把極細接點連到系統板。ABF 是這類載板可使用的增層絕緣膜。'],['pcb','系統 PCB / Server Board','PCB：承載已封裝的晶片和其他元件。CCL 是製造 PCB 的原料，不是另一塊裝在下方的成品板。']];
...
window.SCENES={
 datacenter:{title:'算力的所在地',sub:'運算、網路和電力一起讓 AI 系統運轉。',index:'01 / DATA CENTER',default:'rack',parts:[['rack','AI 運算機櫃','rack'],['switch','網路交換器',''],['hvdc','電力轉換與配電',''],['cooling','CDU 冷卻系統','']]},
 rack:{title:'一櫃，不只是一台電腦',sub:'點選抽屜式運算託盤，繼續認識系統內的部件。',index:'02 / RACK SYSTEM',default:'rack',parts:[['pcb','運算託盤與主機板','board'],['nvlink','互連交換託盤',''],['psu','電源模組',''],['cooling','冷卻液管路',''],['rack','機櫃框架','']]},
 board:{title:'晶片，需要一張連結全局的板',sub:'打開冷板，查看運算與記憶體如何配置。',index:'03 / SERVER BOARD',default:'pcb',parts:[['gpu','GPU 封裝','package'],['memory','CPU／系統記憶體',''],['network','高速連接埠',''],['cooling','晶片冷板',''],['pcb','PCB 板材','materials']]},
 package:{title:'把運算與資料，放在一起',sub:'拉開結構，分清 HBM、中介層和 IC 載板。',index:'04 / ADVANCED PACKAGING',default:'cowos',parts:[['gpu','GPU 裸晶','fab'],['hbm','HBM 堆疊',''],['interposer','中介層',''],['substrate','IC 載板',''],['pcb','系統 PCB','']]},
 materials:{title:'把電路板，拆到材料',sub:'銅箔、玻纖和樹脂，如何做成 CCL，再製成 PCB？',index:'05 / BOARD MATERIALS',default:'ccl',parts:[['copper','銅箔／銅線路',''],['glass','玻纖布骨架',''],['ccl','樹脂與 CCL',''],['drilling','鑽孔與層間連接',''],['dkdf','高速材料電性','']]},
 fab:{title:'從一片晶圓，開始運算的可能',sub:'方格只表示裸晶概念；奈米級電路未依比例顯示。',index:'06 / SILICON WAFER',default:'wafer',parts:[['wafer','矽晶圓',''],['gpu','切分後的裸晶',''],['n2','二奈米製程',''],['asic','ASIC 設計服務','']]}
};
...
window.selectPart=function(id,drill=false){const s=SCENES[window.currentScene];const p=s.parts.find(p=>p[0]===id);if(!termById[id])return;window.activePart=id;if(drill&&!window.mobileExperience&&p?.[2]){setScene(p[2]);return}renderSide(id);window.dispatchEvent(new CustomEvent('partselect',{detail:id}));if(drill)window.dispatchEvent(new CustomEvent('partopen'));};
